...
/* Définir un modèle de diagramme circulaire */
proc template;
	define statgraph SASStudio.Pie;
		begingraph;
		entrytitle "Répartition de la couleur du diamant" / textattrs=(size=14);
		layout region;
		piechart category=color / datalabellocation=callout;
		endlayout;
		endgraph;
	end;
run;
...
proc sgrender template=SASStudio.Pie data=WORK.DIAMONDS2;
run;
